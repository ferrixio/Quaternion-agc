...
Similar to the previous one, this method generates a quaternion whose components are random float extracted with the uniform distribution in [-50,50]. The user can extend (o reduce) these bounds by passing the values `a` and `b`.
